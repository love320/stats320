...
public class Handler {
	...
	public boolean exe(Map<String,Object> map){
		...
		if(config.isIsize()) storage.setString(String.format("%s%d", config.getIndex(),config.isMaster()?1:2), key, map.get(config.getValue()).toString());
		if(!config.isIsize()) storage.setInt(String.format("%s%d", config.getIndex(),config.isMaster()?1:2), key, Integer.valueOf(map.get(config.getValue()).toString()));
		return true;
	}
